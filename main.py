...
def popular_articles():
  ...
  popular_articles_data.append(_pm)
...
def recommended_articles():
    ...
    colNames = ["url", "title", "text", "lang", "total_events"]

    all_recommended = pd.DataFrame(columns=colNames)

    for lm in liked_article:
        output = get_recommendations(lm["title"])
    ...
    recommendedArticleData.append(_pm)

    return jsonify({
      "data":recommendedArticleData,
      "status":"sucess"
  })  
